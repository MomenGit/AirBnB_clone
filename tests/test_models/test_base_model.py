...
class Test_BaseModel(unittest.TestCase):
    ...
    def test_change_updated_at(self):
        ...
        self.assertTrue(self.test_model.created_at < self.test_model.updated_at)
        

    def test__str__(self):
        """Check if __str___ gives the correct format"""
        self.assertEqual(str(self.test_model), "[{}] ({}) {}".format(self.test_model.__class__.__name__,
                                                                self.test_model.id, self.test_model.__dict__))

    def test_args(self):
        """Check if using arguments when crateing an instance it will not be used"""
